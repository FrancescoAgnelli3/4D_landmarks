...
class TemporalGraphODE(Module):
    ...
    def __init__(self, 
        ...
        self.func = TAGConv(in_channels = inp,
                            out_channels = inp,
                            K = self.K,
                            normalize = self.normalization,
                            bias = self.bias)
        
        self.conv = NeuralODE(self.func, sensitivity='adjoint', 
                                    solver=solver, solver_adjoint='euler', 
                                    return_t_eval=False)
    ...
    def forward(self, data: Data, prev_h: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        land_pred = self.conv(h, t_span=self.t_span)
        h = land_pred[-1] # conv returns node states at each evaluation step
